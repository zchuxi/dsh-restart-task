		...
		function apply(ctx) {
			...
			ctx.inject(['configForms'], (fctx) => {
				...
				function createTranscriptPass() {
					...
					/**
					 * The rounds this plugin opened, as far as every window seen has shown.
					 *
					 * A window that pages older rounds away, or a fold that replaces it, cannot
					 * retract one: the set only grows while the transcript stays on the same
					 * session, and starts over when it moves to another one.
					 */
					function ownedTurns() {
