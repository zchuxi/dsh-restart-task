...
/* While the composer holds something the human could send, the round control is
   noise: the message is about to go out, and offering "continue the last broken
   turn" next to it invites the wrong click. The product's own verdict is the
   signal — it disables its primary control exactly when the composer is empty —
   so no bookkeeping is needed here and no keystroke has to be observed: React
   re-renders that attribute on every change and the rule follows. The same rule
   covers the send-button takeover, which clears that attribute while it holds. */
[data-composer-card]:has(button[class*='_primary']:not([disabled])) .dyn-retry-round {
			...
			ctx.inject(['configForms'], (fctx) => {
				...
				fctx.effect(() => {
					...
					const sync = () => {
						...
						// The round control's own "step aside" rule lives in the always-injected
						// stylesheet: it follows the product's primary control, which is true
						// whether or not this plugin is the one that took it over.
				 ...
				 */
				/**
				 * Whether the composer is holding something the human could send.
				 *
				 * `disabled` is the product's own verdict, and it is what the takeover
				 * gates on — but this plugin *clears* that verdict when it takes the
				 * button, so it can no longer be read back to decide whether the human
				 * has since typed. The draft itself has to be looked at, or the takeover
				 * sticks: the composer would keep the orange continue control while a
				 * real message sat in it, and clicking would continue the old task
				 * instead of sending. The editor is one contenteditable root inside the
				 * composer card; attachment chips count as content too.
				 */
				function composerHasDraft(card) {
					if (card === null || card === undefined) return false
					const editor = typeof card.querySelector === 'function' ? card.querySelector('[contenteditable="true"]') : null
					if (editor !== null && editor !== undefined) {
						const raw = typeof editor.textContent === 'string' ? editor.textContent : ''
						// Lexical pads an empty document with zero-width characters.
						if (raw.replace(/[\u200b\u200c\ufeff]/g, '').trim() !== '') return true
					}
					const chips = typeof card.querySelectorAll === 'function'
						? card.querySelectorAll('[class*="_attachment"], [class*="_reference"], [class*="_chip"]')
						: []
					return chips.length > 0
				}

				/** The composer card one primary control belongs to. */
				function cardOf(button) {
					return typeof button.closest === 'function' ? button.closest('[data-composer-card]') : null
				}
				...
				function createTranscriptPass() {
					...
					/**
					 * Hand one button back.
					 *
					 * The product's own verdict for an empty composer is `disabled`, and this
					 * plugin cleared it to make the control clickable — so handing the button
					 * back has to restore it, or the gate can never fire again: the control
					 * would sit enabled-but-idle for the rest of the session and no later
					 * continuation could be offered through it. A composer that *does* hold a
					 * draft keeps whatever the product rendered, because that message is
					 * exactly what must stay sendable.
					 */
					function releaseButton(button) {
						...
						if (composerHasDraft(cardOf(button)) !== true) button.disabled = true
					}
					 ...
					 * typing in must keep sending what they typed. Two independent checks keep
					 * that true — `disabled` when the control is fresh, and the draft itself
					 * for as long as it is held — because taking the button clears the product's
					 * own verdict, and a takeover that outlives the empty composer is worse
					 * than no takeover at all.
					 */
					function applySendButton(state, enabled) {
						...
						for (let i = 0; i < buttons.length; i += 1) {
							...
							if (wanted === true && composerHasDraft(cardOf(button)) !== true) {
				...
				fctx.effect(() => {
					const onClick = (event) => {
						...
						// The decision is re-taken here, not trusted from the last sweep: typing
						// and clicking can land inside one debounce window, and a message the
						// human wrote must never be turned into a continuation.
						if (composerHasDraft(cardOf(button)) === true) return
